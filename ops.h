...
typedef struct {
    ...
    uint8_t (*execute)(cpu_6502_t *cpu, uint8_t operand); 
    /* function pointer to address mode */
    uint8_t (*addr_mode)(cpu_6502_t *cpu);             
} op_t;

/* address modes */
uint8_t am_imp(cpu_6502_t *cpu);
...
uint8_t op_xxx(cpu_6502_t *cpu, uint8_t operand);
